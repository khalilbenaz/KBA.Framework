...
public class AuthController : ControllerBase
{
    ...
    public async Task<ActionResult<AuthResponseDto>> Login([FromBody] LoginDto loginDto, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(ex, "Échec d'authentification pour l'utilisateur: {UserName}", loginDto.UserName);
            return Unauthorized(new { message = ex.Message });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Erreur lors de l'authentification de l'utilisateur: {UserName}", loginDto.UserName);
