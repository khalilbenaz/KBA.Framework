namespace KBA.Framework.Domain.Entities.MultiTenancy;
...
public class Tenant : FullAuditedEntity<Guid>
{
    ...
    public Tenant(string name, Guid? userId = null)
    {
        ...
        if (name.Length > KBAConsts.MaxNameLength)
            throw new ArgumentException($"Le nom du tenant ne peut pas dépasser {KBAConsts.MaxNameLength} caractères.", nameof(name));
